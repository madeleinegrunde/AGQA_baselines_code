...
class Encoder(nn.Module):
    ...
    def forward(self, src_seq, return_attns=False): # src_seq: batch_size x steps x ctx_dim
        ...
        enc_input = src_seq + self.pos_linear(pos_emb)  # position embedding error
        enc_input = self.dropout(enc_input) # NEW DROPOUT
...
class VQAttention(nn.Module):
    ...
    def forward(self, Vid_enc, Ques_enc, V_mask, Q_mask):
        ...
        out = F.dropout(out, p=dropout, training=self.training) # NEW DROPOUT
