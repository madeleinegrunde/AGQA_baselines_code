...
# //////////// changed this to reflect metric, and added in str2num
def getTypeToCSV(metric):
    TYPE_TO_CSV = {'FrameQA': 'Train_frameqa_question-%s.csv' % metric,
...
class DatasetTGIF():
    ...
    def __init__(self,
        ...
        self.data_df = self.read_df_from_csvfile(metric)
        self.GLOVE_EMBEDDING_SIZE = 300
        print('building dataset for metric', self.metric)
...
#            random.shuffle(self.ids)
    ...
    def read_df_from_csvfile(self, metric):
    ...
    def build_word_vocabulary(self, all_captions_source=None,
        ...
        with open(self.vocabulary_dir + '/glove.42B.300d.txt','r') as f:
            lines = f.readlines()
        dict = {}
    ...
    def iter_ids(self, shuffle=True):
    ...
    def get_all_captions(self):
        ...
        # this gets metric
	TYPE_TO_CSV = getTypeToCSV(self.metric)
    ...
    def load_video_feature(self, key):
        ...
        # change feature file paths
            ...
            with h5py.File('../../TGIF_RESNET_pool5.hdf5', 'r') as f_app: # TODO: PATH
                resnet_feature = f_app['resnet_features'][video_id]  # (8, 16, 2048)
            with h5py.File('../../TGIF_RESNEXT.hdf5', 'r') as f_app: # TODO: PATH
    ...
    def get_question_answer(self, key):
        '''
        Return question and answer strings for given key.
        '''
        question = self.data_df.loc[self.data_df['key'] == key, ['question']].values
        answer = self.data_df.loc[self.data_df['key'] == key, ['answer']].values
        question = question[0,0]
        answer = answer[0,0]
        question = self.convert_sentence_to_matrix(question, eos=False)
    ...
    def get_FrameQA_result(self, chunk):
        ...
        for k in xrange(batch_size):
            key = chunk[k]
            video_feature = self.get_video_feature(key[:5])
            ...
            question, answer = self.get_question_answer(key)            
            answer_type = 0
            ...
            batch_debug_sent[k] = self.data_df.loc[self.data_df['key'] == key, 'question']
    ...
    def batch_iter(self, num_epochs, batch_size, shuffle=False, sample=True):
        for epoch in xrange(num_epochs):
            steps_in_epoch = int(len(self) / batch_size)
            if sample: 
                steps_in_epoch = int(10000. / batch_size)
	        print("running ", steps_in_epoch, "batches")
